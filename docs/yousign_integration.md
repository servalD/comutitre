...
4. Tunnel local HTTPS pour les webhooks en dev (ngrok ou cloudflared)

---

## Variables d'environnement (back/.env.dev)
...
1. Exposer le backend localement :
   ```bash
   ngrok http 3000
   # ou
...
```bash
# 1. Migrations
cd back && pnpm migration:run

# 2. Seed démo (optionnel)
pnpm seed:demo
